...
| `SessionEntity` / `ProgrammeGroupMembershipEntity` | `attendances: MutableSet<SessionAttendanceEntity>` | **`SessionService.kt:560` `attendances.maxWithOrNull(compareBy { it.createdAt }.thenBy { it.id })`** (surfaced by PR #877 9-eyes review, 2026-08-24) — picks the "current" attendance for a referral in `RecordSessionAttendance` payload construction | **UUID tiebreak — same anti-pattern as override #6.** If two attendances tie on `createdAt` (e.g. two recordings in the same second, or a bulk retroactive backfill), `.thenBy { it.id }` gives a UUID-lottery-dependent result → "current attendance state" can flip between JVM/DB runs. **Functional impact:** the user-facing "current attendance" for a referral could flip. Recommend natural-attribute tiebreak: `.thenBy { it.recordedAt }.thenBy { it.recordedByFacilitator?.id }` or restrict to unique `(sessionId, groupMembershipId)` at DB level. Verify against `SessionAttendanceEntity` schema before proposing a fix. |
| `SessionEntity` / `ProgrammeGroupMembershipEntity` | `attendances: MutableSet<SessionAttendanceEntity>` | **`ProgrammeGroupService.kt:746` `.sortedWith(compareByDescending { it.createdAt }.thenByDescending { it.id }).firstOrNull()`** (surfaced by PR #877 9-eyes review, 2026-08-24) — picks the latest attendance per attendee for `AttendanceAndSessionNotes` construction | Same UUID-lottery concern as `SessionService.kt:560`. Note the tiebreak here is **`thenByDescending { it.id }`** (not `thenBy`) — the direction is consistent with the primary `createdAt` DESC, but the concern is identical: UUID lottery, not natural attributes. Both call-sites likely want the same fix; consider a shared helper (e.g. `Iterable<SessionAttendanceEntity>.latestByCreatedAt(): SessionAttendanceEntity?`) to consolidate the sort logic once and prevent divergence. |

**Implementer:** treat this table as a starting sketch, not gospel. Re-grep and re-verify every row before adjusting anything.

**Reference implementation for the UUID-tiebreak → natural-attribute-tiebreak fix pattern:** PR #877 commit `45a4b03d` fixed the same anti-pattern on the SAR DTO mapper (`SubjectAccessRequestReferral.kt`). Read that commit for the exact code shape — mapper uses natural attributes (`programmeGroup.code`, `hasLdc`+`createdBy`, `createdBy`+`cohort.name`) instead of `.thenBy { it.id }`, and the calling test seeds distinct `createdAt` values per row. The same shape applies here.
