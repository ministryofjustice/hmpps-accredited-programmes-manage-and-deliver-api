...
| `ReferralEntity` | `referralCohortHistories` | `Referral.kt` model `maxByOrNull { it.createdAt }`, `TelemetryService.firstOrNull()`, tests use `.first()` and `.maxByOrNull`, DTO mapper (now sorted DESC) | `TelemetryService.firstOrNull()` reads insertion-order (`.firstOrNull()` on LinkedHashSet); if the first inserted row is meant to be the OLDEST, this is intent-correct but fragile. Recommend explicit sort. |
