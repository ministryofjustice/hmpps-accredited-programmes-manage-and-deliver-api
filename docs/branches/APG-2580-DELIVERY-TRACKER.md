...
5. **Downstream impact** — Confirmed via grep. No files outside those listed in Branch docs reference: `SubjectAccessRequestGroupWaitlistItemView`, `SubjectAccessRequestReferralCaseListItemView`, `outcomeTypeCode`. Factories (`GroupWaitlistItemViewEntityFactory`, `ReferralCaseListItemViewEntityFactory`) stay in place — used by other non-SAR tests.
