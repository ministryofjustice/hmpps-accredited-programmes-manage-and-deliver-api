...
| 1.5 sweep-up `APG-2580/cleanup-orphaned-sar-factories` | [Sweep-up plan](./APG-2580-cleanup-sar-factories-plan.md) | ⏳ Ready to start (prompt below) — PR #860 merged as `4adc4b08` | — | — | Reduced-scope tidy-up PR. Fixes 1 pre-existing `List<String?>` vs `List<OffenceCohort>` assertion in `SubjectAccessRequestServiceTest.kt`. **Factory deletions were dropped from scope** after implementer preflight (2026-08-20) — `GroupWaitlistItemViewEntityFactory` was picked up by PR #865 within a day of being marked orphaned; `ReferralCaseListItemViewEntityFactory` kept to avoid premature removal (see plan doc § Scope reduction and correction #7). Branch exists on origin at `98e49d7a` (plan doc only). Does NOT block Branch 2 — they can run in parallel. |
