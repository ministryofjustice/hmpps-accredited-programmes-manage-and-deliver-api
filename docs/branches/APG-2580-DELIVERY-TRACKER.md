...
|---|--------|-----|----------|-----------|
| F1 | `APG-2580/repin-sar-json-fixture` | [followup-1](./APG-2580-followup-1-repin-sar-json-fixture.md) | **HIGH** | Removes the "known CI pin drift" that every Branch since Branch 3 has been carrying as override #6. Rooted the flake in a UUID-based `.thenBy { it.id }` tiebreak on tied `createdAt` values in the SAR contract seed. Plan: natural-attribute tiebreak in mapper + distinct `createdAt` in seed. Deletes override #6. |
...
| F4 | `APG-2580/entity-set-iteration-audit` | [followup-4](./APG-2580-followup-4-entity-set-iteration-audit.md) | Low | One-time audit of every `MutableSet<X>` field on `@Entity` classes for order-sensitive consumers (e.g. `TelemetryService.referralCohortHistories.firstOrNull()` — insertion-order-dependent, no explicit sort). Direct follow-up of Correction #8. |
