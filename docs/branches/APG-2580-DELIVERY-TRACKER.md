...
| 3. `APG-2580/enum-display-names` | [Branch 3](./APG-2580-branch-3-enum-display-names.md) | 🔎 Awaiting rebase + human review | [#872](https://github.com/ministryofjustice/hmpps-accredited-programmes-manage-and-deliver-api/pull/872) | — | Base: `main` (pre-#871). 10 files: 4 enums gain `displayName`, 3 SAR mappers switched to display strings, `SubjectAccessRequestServiceTest` 5 assertions updated, both fixtures regenerated. All six field transitions visible in JSON + HTML. **Rebase pending on updated `main`** (post-#871) — current fixture still shows `"outcomeTypeCode":"ATTC"`; needs `regenerate-sar-snapshots.sh` after rebase so Branch 2's field rename is reflected. Undisclosed style deviation: `SettingType.PRE_RELEASE("Pre Release")` shipped instead of plan doc's `"Pre-release"` — matches the family style but wasn't explicitly signed off; flagged for reviewer confirmation. Cohort-ordering flake also baked one race outcome into the fixture (pre-existing; Branch 5 / follow-up ticket territory). |
