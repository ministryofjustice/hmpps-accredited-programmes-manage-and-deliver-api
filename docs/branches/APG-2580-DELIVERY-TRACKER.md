...
| [`APG-2580-followup-1-repin-sar-json-fixture.md`](./APG-2580-followup-1-repin-sar-json-fixture.md) | **HIGH priority** — kill the CI-pinned cohort-ordering flake (removes tracker override #6) |
...
| 5. `APG-2580/render-test-seed-attendance-row` (follow-up) | [Branch 5](./APG-2580-branch-5-render-test-seed-attendance-row.md) | 🔎 In review | [#877](https://github.com/ministryofjustice/hmpps-accredited-programmes-manage-and-deliver-api/pull/877) | — | Scope rewritten 2026-08-24 — DTO type fix (3× `MutableSet` → `MutableList` + sort safeguards). Nine-eyes review clean (see PR body). Comment-only follow-up commit `490aa95b` adds sort-direction rationale on all three mappers. Local `SarContractIntegrationTest` cohort-ordering failure is the pre-existing pin drift documented as override #6 — resolved in [followup-1](./APG-2580-followup-1-repin-sar-json-fixture.md). Awaiting CI + human review. |
...
|---|--------|-----|----------|-----------|
| F1 | `APG-2580/repin-sar-json-fixture` | [followup-1](./APG-2580-followup-1-repin-sar-json-fixture.md) | **HIGH** | Removes the "known CI pin drift" that every Branch since Branch 3 has been carrying as override #6. **Doc includes a mandatory Step 0 root-cause reproduction step** (two hypotheses: H1 tied AUTH_USER `createdAt`s / H2 async race in `refreshPersonalDetailsForReferral`) — implementer must prove which is real before applying any fix. If H1: Strategy A' (natural-attribute mapper tiebreak using `programmeGroup.code` NOT `.id`, plus distinct `createdAt` per cohort row in the seed). If H2: re-scope to coroutine-race fix and re-plan. Deletes override #6 on H1 success. |
...
- **Timezone/locale sanity on CI:** Branch 5 regenerated `sar-expected-render-result.html` locally. Any date rendering that depends on JVM default locale or timezone could produce a diff on CI even without any code change. First CI run after each SAR-fixture-touching PR should be eyeballed for this. If CI diverges from local, the fix is to explicitly configure `formatDate` with a fixed locale/zone — do not re-pin to whatever CI produced (same anti-pattern as override #6). Filed under followup-1 concerns but applies generally.
...
- **Post-Step-0 discovery risk on followup-1:** if Step 0 confirms H2 (async race), followup-1's Strategy A' is **not the correct fix**. See the doc's "Fallback (H2 confirmed)" section. Planning agent must be notified before any re-scope; do not silently switch strategies.
