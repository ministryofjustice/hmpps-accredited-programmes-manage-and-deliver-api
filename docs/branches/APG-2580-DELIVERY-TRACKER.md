...
**Last updated:** 2026-08-24 (Branch 5 rescoped after root-cause investigation)
...
| [`APG-2580-followup-1-repin-sar-json-fixture.md`](./APG-2580-followup-1-repin-sar-json-fixture.md) | ✅ **Delivered via PR #877 commit `45a4b03d`** — kept as historical record of Strategy A' analysis and Step 0 methodology |
| [`APG-2580-followup-2-sar-set-footgun-guardrails.md`](./APG-2580-followup-2-sar-set-footgun-guardrails.md) | Low priority — architecture test (Part A). Optional Part B mustache-helper sweep is **significantly harder than initially thought** (rendering engine lives in `hmpps-kotlin-spring-boot-starter` — cross-repo change or fragile bean override) — recommend Part A only |
| [`APG-2580-followup-3-sar-fixture-drift-guardrail.md`](./APG-2580-followup-3-sar-fixture-drift-guardrail.md) | Low priority — CI gate that regens SAR fixtures and fails on drift |
| [`APG-2580-followup-4-entity-set-iteration-audit.md`](./APG-2580-followup-4-entity-set-iteration-audit.md) | Low priority — one-time audit of every `MutableSet` field on JPA entities for order-sensitive consumers |
| [`APG-2580-followup-4-entity-set-iteration-audit-report.md`](./APG-2580-followup-4-entity-set-iteration-audit-report.md) | The audit **report** (findings, 16 fields catalogued, verdict per field, recommended follow-ups). Produced by the audit branch (PR #880). Use this as the source of truth for what F5/F6 must cover. |
| [`APG-2580-followup-5-attendance-latest-consolidation.md`](./APG-2580-followup-5-attendance-latest-consolidation.md) | Low–Medium priority — route 5 attendance-latest read sites through `latestByCreatedAt()` + add sibling `latestByRecordedAtOrCreatedAt()` for `NDeliusAppointmentEntity.currentAttendance`. Blocked by PR #880 merge. |
| [`APG-2580-followup-6-membership-and-status-history-tiebreaks.md`](./APG-2580-followup-6-membership-and-status-history-tiebreaks.md) | Medium (Part A) / Low (Part B) priority — add `@OrderBy("createdAt DESC")` + in-Kotlin tiebreak to `programmeGroupMemberships` reads (Part A, functional — drives referral-completion event); mirror the cohort tiebreak fix onto `TelemetryService.statusHistories` (Part B, symmetry). Independent of F5. |
...
9. **SAR HTML template collapses `MutableSet` collections to "No Data Held" (Branch 5 root-cause discovery, 2026-08-24)** — Planning-agent investigation while drafting Branch 5 uncovered a pre-existing production bug that has affected every Community SAR since PR #644. The SAR mustache template uses `{{#field.0}}` (list-index access at position 0) as the "is-non-empty" guard for its collection sections. Kotlin `MutableSet` does not support `[0]` / `get(0)` access, so JMustache evaluates `field.0` on a Set as falsy → the `{{^field.0}}<p>No Data Held</p>{{/field.0}}` fallback always fires, even when the Set has entries. **Empirically confirmed pattern on `main`:** `attendees` (`MutableList`) and `statusHistories` (`MutableList`) render correctly; `programmeGroupMemberships`, `referralCohortHistories`, and `referralLdcHistories` (all `MutableSet` on the DTO) render as "No Data Held" despite the JSON fixture containing real data for all three. JSON is unaffected because Jackson serialises Sets and Lists identically as arrays; only the mustache-rendered HTML/PDF is broken. **Impact:** Real subjects receiving Community SARs have been getting reports missing their Programme Group Memberships, Cohort History, and LDC History sections. **Fix (Branch 5):** Change the three DTO field types from `MutableSet<...>` to `MutableList<...>` in `SubjectAccessRequestReferral.kt`; swap the mapper's `.toMutableSet()` calls to `.toMutableList()`; add `.sortedWith(...)` calls to two of the three fields (Correction #8 pattern) for deterministic ordering. Total diff: ~10 lines of production code + two regenerated fixtures. Full plan in [`APG-2580-branch-5-render-test-seed-attendance-row.md`](./APG-2580-branch-5-render-test-seed-attendance-row.md). **This also invalidates the earlier "seed enrichment" scoping for Branch 5** — the seed was always producing data; the bug was that the render pipeline was throwing it away.
...
| 5. `APG-2580/render-test-seed-attendance-row` (follow-up) | [Branch 5](./APG-2580-branch-5-render-test-seed-attendance-row.md) | 🔎 In review | [#877](https://github.com/ministryofjustice/hmpps-accredited-programmes-manage-and-deliver-api/pull/877) | — | Scope expanded 2026-08-24 after CI failed 4× — original DTO type fix (3 `MutableSet` → `MutableList`) + **followup-1's Strategy A' pulled in** (commit `45a4b03d`) because the CI failure was override #6 drift proven live. Fix: mapper uses natural-attribute tiebreaks (`programmeGroup.code`, `hasLdc`+`createdBy`, `createdBy`+`cohort.name`) replacing UUID `.thenBy { it.id }`; seed `SarContractIntegrationTest.kt` gives every cohort row a distinct `createdAt`. Local verification: 878/878 build tests, 4/4 SarContract tests, 3× BST + 1× UTC re-runs all deterministic. Comment-only commit `490aa95b` (sort rationale) preceded. **Override #6 resolved by this PR.** Awaiting CI + human review. |

---

## Post-APG-2580 follow-up branches

Discovered during Branches 1–5. These are **not part of APG-2580's scope** but each has a full plan doc with an implementer prompt ready to hand off to a fresh chat session.

| # | Branch | Doc | Priority | Rationale |
|---|--------|-----|----------|-----------|
| F1 | ~~`APG-2580/repin-sar-json-fixture`~~ | ~~[followup-1](./APG-2580-followup-1-repin-sar-json-fixture.md)~~ | ✅ **DELIVERED via PR #877** | Originally scoped as a separate branch. Pulled into Branch 5's PR #877 on 2026-08-24 when CI failed 4× — the failure IS this ticket's target and blocking Branch 5 merge. Step 0 reproduced live: `TZ=UTC` regen produced `[AUTH_USER,AUTH_USER,SYSTEM]` cohort order vs committed `[AUTH_USER,SYSTEM,AUTH_USER]` — H1 classification (only reordering, no row-count change → not H2's async-race pattern). Applied Strategy A' from the plan doc verbatim (mapper natural-attribute tiebreak using `programmeGroup.code` NOT `.id`, plus distinct `createdAt` per cohort row in the seed). Commit `45a4b03d`. Local verification: 878-test full build + 4/4 SarContract tests + 3× BST + 1× UTC reruns, all deterministic. **Override #6 resolved in the same PR.** The followup-1 plan doc stays in the repo as historical record of the analysis + Strategy A' rationale. |
| F2 | `APG-2580/sar-set-footgun-guardrails` | [followup-2](./APG-2580-followup-2-sar-set-footgun-guardrails.md) | Low | Architecture test that fails at build time if any SAR DTO field is `Set`/`MutableSet` (preventing recurrence of Correction #9). Part B (optional mustache helper sweep) is **significantly harder than initially thought** — template rendering happens in `hmpps-kotlin-spring-boot-starter:3.0.0` library, so Part B requires either upstreaming to that library (B1) or a fragile Spring bean override (B2). Recommend Part A only. |
| F3 | `APG-2580/sar-fixture-drift-guardrail` | [followup-3](./APG-2580-followup-3-sar-fixture-drift-guardrail.md) | Low | CI gate that regens all **three** SAR fixtures (`entity-schema-snapshot.json`, `sar-api-response.json`, `sar-expected-render-result.html`) and fails on any diff. Would have caught Correction #9 automatically. CI provider verified as GitHub Actions (not CircleCI). |
| F4 | `APG-2580/entity-set-iteration-audit` | [followup-4](./APG-2580-followup-4-entity-set-iteration-audit.md) | Low | One-time audit of every `MutableSet<X>` field on `@Entity` classes for order-sensitive consumers. **Confirmed at-risk finding pre-catalogued: `TelemetryService.kt:39` reads `referralCohortHistories.firstOrNull().cohort` (insertion-order-dependent).** Direct follow-up of Correction #8. |
| F5 | `APG-2580/attendance-latest-consolidation` | [followup-5](./APG-2580-followup-5-attendance-latest-consolidation.md) | Low–Medium (blocked by F4/PR #880 merge) | Route the 5 additional attendance-latest reads (`ReferralService.kt:688`, `SessionService.kt:461,617`, `NDeliusAppointmentEntity.currentAttendance()`, `SessionNotes.kt:105`) through `latestByCreatedAt()`. Adds one sibling helper `latestByRecordedAtOrCreatedAt()` for the `NDeliusAppointmentEntity` site whose selector must preserve `recordedAt` semantics. ~½ day. |
| F6 | `APG-2580/membership-and-status-history-tiebreaks` | [followup-6](./APG-2580-followup-6-membership-and-status-history-tiebreaks.md) | **Medium (Part A)** / Low (Part B) — independent of F5 | **Part A:** add `@OrderBy("createdAt DESC")` to `ReferralEntity.programmeGroupMemberships` + explicit `.sortedWith(...)` tiebreak at the two `ReferralStatusService.kt:113,157` consumer sites (functional — drives `ReferralProgrammeCompleteEvent` publication). **Part B:** mirror the `referralCohortHistories` fix from PR #880 onto `TelemetryService.statusHistories` for `referralStatus` + `fromStatus` (`MutableList` w/ `@OrderBy`, symmetry only). ~2 hours combined. |

### Standing reminders (do not forget)

- **Timezone/locale sanity on CI:** ✅ **Ruled out 2026-08-24** — regenerated `sar-expected-render-result.html` under `TZ=UTC` locally and diff was byte-identical to `TZ=BST` regen. The `@BeforeEach` `clock.zone → ZoneOffset.UTC` mock at `SarContractIntegrationTest.kt:357` fully isolates the SAR render from JVM default timezone. If future SAR-render-touching PRs hit CI-only diffs, TZ can be quickly ruled out with `TZ=UTC bash scripts/local-scripts/regenerate-sar-snapshots.sh && git diff src/test/resources/sar/`. Keeping this note as the recipe.
- **followup-3 and followup-4 kept as separate docs (not consolidated):** the planning agent's initial thought was to merge item #5 (fixture drift guardrail) and item #6 (entity Set iteration audit) into a single "misc hardening" doc. On drafting they turned out to address different bug classes (fixture pipeline vs entity iteration ordering) with different implementer scopes, so they were kept separate. If future planning wants to bundle them into a single delivery PR that's fine, but the plan docs stay separate for clarity of scope.
- **Post-Step-0 discovery risk on followup-1:** ✅ **Resolved 2026-08-24** — Step 0 was executed empirically on PR #877 (regen-diff experiment), H1 confirmed, Strategy A' applied. See F1 row above.
...
3. **Mandatory pre-flight greps** (added after Branch 1/2/3 discoveries) — before you commit, run:
   ```bash
   grep -rn "SessionAttendance\|SessionNoteHistory" src/test/ | grep -iE "createdBy|createdAt" | grep -v "\.log:"
   grep -rn "attendance\.createdBy\|attendance\.createdAt\|noteHistory\[.\]\.createdBy" src/
   ```
   Any hit outside your changed files or the SAR fixture is a downstream assertion that needs updating. Also grep `HmppsSubjectAccessRequestControllerIntegrationTest.kt` for the field names `createdBy`, `createdAt` on the attendance/note path — Branch 1 showed this file occasionally has key-presence / value assertions that need updating even though the DTO shape isn't changing.
...
```bash
git add -A
git commit --no-gpg-sign -m "APG-2580: Use display names for enum-valued SAR fields"
...
## Implementer prompt – Branch 4: Attendance & session-note restructure

**Copy everything from `>>>>> BEGIN PROMPT` down to `<<<<< END PROMPT` into a fresh agent session.**

>>>>> BEGIN PROMPT

You are an implementation agent delivering branch 4 (the final code branch) of Jira ticket **APG-2580** in the repo `hmpps-accredited-programmes-manage-and-deliver-api`. The branch has been fully planned already — your job is to execute the plan exactly, run the tests, and report back.

**Repo:** `/Users/raby.whyte/code/hmpps-accredited-programmes-manage-and-deliver-api`

### Setup

Branches 1, 1.5 sweep-up, 2, and 3 have all merged to `main`. Base this branch on `main`:

```bash
git fetch origin
git checkout main && git pull
git checkout -b APG-2580/attendance-session-note-restructure
```
...
`docs/branches/APG-2580-branch-4-attendance-session-note-restructure.md`

That doc modifies exactly 3 files:

1. **`SubjectAccessRequestSessionAttendance.kt`** — remove `createdBy: String` and `createdAt: LocalDateTime` from the data class and from `toApi()`. The `LocalDateTime` import stays because `recordedAt: LocalDateTime?` still uses it.
2. **`SubjectAccessRequestSessionNoteHistory.kt`** — replace `createdBy: String` with `recordedBy: String?`; mapper now reads `createdByFullName` from the entity instead of `createdBy`.
3. **`sar_template.mustache`** — restructure the attendance section: per-attendance `<div class="attendance-record">` with a `summary-list` table (Outcome, Legitimate Absence, Recorded By, Recorded At — no `Created By` / `Created At`); notes table now nested *inside* each attendance div; notes columns updated to `Recorded By` (using `{{ optionalValue recordedBy }}`) and `Recorded At` (still binds `createdAt`).

### Confirmed product decisions (do NOT re-ask)

- **Note `recordedBy` = `null` / "No Data Held" in the fixture is acceptable.** Signed off 2026-08-21. The integration test factory doesn't populate `createdByFullName`, so the regenerated JSON will show `"recordedBy":null` and the HTML render will show `"No Data Held"`. Production data will have real values.
...
1. **Fixture regeneration:** ignore the manual `SAR_GENERATE_ACTUAL=true ./gradlew …` + `cp` sequence in the plan. Run this instead:
   ...
   ./scripts/local-scripts/regenerate-sar-snapshots.sh
   ```
2. **Mandatory pre-flight greps** (added after Branch 1/2/3 discoveries) — before you commit, run:
   ```bash
   grep -rn "SessionAttendance\|SessionNoteHistory" src/test/ | grep -iE "createdBy|createdAt" | grep -v "\.log:"
   grep -rn "attendance\.createdBy\|attendance\.createdAt\|noteHistory\[.\]\.createdBy" src/
   ```
   Any hit outside your changed files or the SAR fixture is a downstream assertion that needs updating. Also grep `HmppsSubjectAccessRequestControllerIntegrationTest.kt` for the field names `createdBy`, `createdAt` on the attendance/note path — Branch 1 showed this file occasionally has key-presence / value assertions that need updating even though the DTO shape isn't changing.
3. **Known CI flake tolerance** — the `SarContractIntegrationTest` occasionally fails on the first `./gradlew build` with a `referralCohortHistories` ordering diff. This is a pre-existing async race between the test seed and `refreshPersonalDetailsForReferral` — NOT introduced by your changes. Branch 3 landed a fixture pin for CI's stable ordering (commit `158cb481`), which should hold for you too. If the flake fires despite the pin, rerun `./gradlew build` once; do NOT try to fix at source (tracked as a standalone follow-up ticket).
4. **Correction #7 (test-factory orphan checks)** — do not delete any test factory as part of this branch. Orphan status flipped in <24h during the 1.5 sweep-up.
5. **Correction #8 pattern** — you are restructuring collection-valued SAR fields (`noteHistory`, and by knock-on the mustache rendering of `attendances`). The pattern is safe here because neither collection is a raw `MutableSet` — `SessionAttendanceEntity.notesHistory` is a `List` and `SessionAttendanceEntity` collections come out of the entity in insertion/JPA order, which is deterministic. **But:** if during your edits you find yourself introducing any new `MutableSet<...>` → serialised path, apply an explicit `.sortedWith(...)` before `.toMutableSet()` in the mapper, following the Branch 1 pattern in `SubjectAccessRequestReferral.toApi()`.
6. **Render-test coverage gap warning** — the SAR render-test seed currently collapses Programme Group Memberships to "No Data Held" for attendance/notes, so the mustache restructure will NOT surface in `sar-expected-render-result.html`. This is a pre-existing gap being handled separately as Branch 5 (planned to execute after this branch). Your JSON-fixture diff is the primary contract; the HTML-render diff is expected to be minimal or empty for the attendance/note section. Do NOT try to enrich the render-test seed in this branch — that is Branch 5's job and doing it here compounds risk with the DTO restructure.

### Acceptance criteria (all must be true before you report back)

- [ ] `SubjectAccessRequestSessionAttendance.kt` — `createdBy` and `createdAt` fields removed from the data class *and* from `toApi()`; `LocalDateTime` import retained (still used by `recordedAt`); `UUID` import retained.
- [ ] `SubjectAccessRequestSessionNoteHistory.kt` — data class field is `recordedBy: String?` (nullable); `toApi()` sets `recordedBy = createdByFullName`.
- [ ] `sar_template.mustache` — attendance section matches the "New structure" block in the plan doc verbatim: one `<div class="attendance-record">` per attendance, notes nested inside the div, no `Created By`/`Created At` columns anywhere in the attendance section, notes columns are `Recorded By` / `Recorded At`.
- [ ] Pre-flight greps clean (no downstream test assertions on the removed fields outside your changed files).
- [ ] `./gradlew ktlintCheck` passes.
- [ ] `./scripts/local-scripts/regenerate-sar-snapshots.sh` completes; both SAR fixtures change.
- [ ] `sar-api-response.json` diff shows:
  - Attendance object no longer contains `"createdBy":"UNKNOWN_USER"` or `"createdAt":"<DATE_TIME>"` at the top level of the attendance record.
  - `noteHistory[]` object shows `"recordedBy":null` (not `"createdBy":"UNKNOWN_USER"`).
  - No unexpected reordering elsewhere; if `referralCohortHistories` shifts, that's the pre-existing race (see override #3).
- [ ] `sar-expected-render-result.html` diff may be minimal because of the render-test coverage gap (override #6) — that's expected. If any visible changes appear (e.g. because the seed emits attendance data unexpectedly), they should match the plan's "New structure" block.
- [ ] `./gradlew test --tests "*SarContractIntegrationTest*"` passes without `SAR_GENERATE_ACTUAL` (rerun once if the known cohort-ordering flake hits — see override #3).
- [ ] `./gradlew build` passes (877+ tests, 0 failures on final run).

### Push and open PR

```bash
git add -A
git commit --no-gpg-sign -m "APG-2580: Restructure attendance and session-note SAR fields"
git push -u origin APG-2580/attendance-session-note-restructure
```

Open a PR against `main` titled: `APG-2580 Restructure attendance and session-note SAR fields (community)`

Include in the PR description:
- Stacks on PRs #860, #868, #871, #872, #874 (all merged).
- Product sign-off for note `recordedBy` = "No Data Held" in the test fixture recorded 2026-08-21.
- Note that the HTML render-test may be minimal / empty in the attendance section owing to a pre-existing render-test seed gap tracked as Branch 5.
...
PRE-FLIGHT GREP RESULTS:
  - test-side references to attendance createdBy/createdAt outside changed files: <list any or "none">
  - HmppsSubjectAccessRequestControllerIntegrationTest references: <list or "none">
...
FIXTURE DIFF SUMMARY:
  - sar-api-response.json: <one line: what changed in the attendance / noteHistory objects>
  - sar-expected-render-result.html: <one line: what changed, or "unchanged / minimal due to render-test seed gap">
...
Do NOT proceed to Branch 5 (it is a follow-up handled separately). Stop after reporting.

<<<<< END PROMPT

---

## Implementer prompt – Branch 5: Fix SAR sections silently collapsing to "No Data Held"

**Copy everything from `>>>>> BEGIN PROMPT` down to `<<<<< END PROMPT` into a fresh agent session.**

>>>>> BEGIN PROMPT

You are an implementation agent delivering branch 5 (the final branch) of Jira ticket **APG-2580** in the repo `hmpps-accredited-programmes-manage-and-deliver-api`. The branch has been fully planned already — your job is to execute the plan exactly, run the tests, and report back.
...
The branch name is a legacy holdover — the actual scope is a DTO type fix, not a seed enrichment. See the plan doc for the history.
...
That plan changes exactly **one production file** (`SubjectAccessRequestReferral.kt`) plus two regenerated fixtures. Total production diff: ~10 lines.

### Context — what you are fixing

Real Community SARs delivered to subjects have been showing `<p>No Data Held</p>` for three sections since PR #644, even when data exists. Root cause is documented in tracker Correction #9: the SAR mustache template uses `{{#field.0}}` (list-index access) as its "is-non-empty" guard, but the affected fields are `MutableSet` on the DTO. Kotlin `MutableSet` does not support `[0]` access, so JMustache always takes the fallback branch and emits "No Data Held". The three sections are:

- Programme Group Memberships (field: `programmeGroupMemberships`)
- Cohort History (field: `referralCohortHistories`)
- LDC History (field: `referralLdcHistories`)

The JSON SAR endpoint has always been correct (Jackson serialises Sets and Lists identically as arrays). Only the mustache-rendered HTML/PDF report was broken.
...
2. **Do NOT touch the mustache template.** The `{{#field.0}}` idiom will start working correctly once the fields are `List`-typed. Any mustache edit widens the blast radius unnecessarily.
3. **Do NOT touch entity classes.** The DTO change alone is sufficient. The underlying JPA collections remain `MutableSet` at the entity level — that is correct for JPA and does not need changing.
4. **JSON fixture is expected to have a minimal diff.** Sets and Lists both serialise as JSON arrays, so shape is identical. Only ordering may shift for `programmeGroupMemberships` and `referralLdcHistories` because the new `.sortedWith(...)` calls impose a stable order. `referralCohortHistories` is pinned to CI's order by Branch 3's `158cb481` and should stay put.
5. **Ordering safety (Correction #8):** MUST add `.sortedWith(compareBy...thenBy { it.id })` for `programmeGroupMemberships` and `referralLdcHistories` before their `.map { it.toApi() }.toMutableList()` call. The plan doc lists suggested directions (ASC for `programmeGroupMemberships`, DESC for `referralLdcHistories`). Do NOT change the existing sort on `referralCohortHistories` — it already follows the pattern from Branch 1.
6. **Known CI cohort-ordering flake:** `SarContractIntegrationTest` occasionally fails on the first `./gradlew build` with a `referralCohortHistories` ordering diff. Pre-existing async race (Correction #8). Rerun once if it fires; do NOT edit the JSON fixture — the CI pin from `158cb481` should hold.
7. **No CSS work.** The SAR template has zero `<style>` blocks, zero `style=` attributes, and zero CSS files anywhere in the repo. Unstyled semantic markers are project convention (Branch 4 review audit, 2026-08-24).
8. **No seed enrichment.** The current seed already produces one attendance + one note + one cohort history + one LDC history + one programme group membership — enough to prove all three sections render correctly after the fix. Do not add more rows.

### Acceptance criteria (all must be true before you report back)

- [ ] `SubjectAccessRequestReferral.kt` — three DTO field types changed from `MutableSet<...>` to `MutableList<...>` (`programmeGroupMemberships`, `referralLdcHistories`, `referralCohortHistories`). Other collections (`statusHistories`, `messageHistories`, `attendees`) untouched.
- [ ] `SubjectAccessRequestReferral.kt` — three mapper `.toMutableSet()` calls changed to `.toMutableList()`.
- [ ] `SubjectAccessRequestReferral.kt` — `.sortedWith(...)` calls added for `programmeGroupMemberships` and `referralLdcHistories`; the existing `.sortedWith(...)` on `referralCohortHistories` is unchanged.
- [ ] Necessary entity-class imports added if not already present.
- [ ] Pre-flight greps (see plan doc § "Pre-flight greps") return no unexpected hits.
- [ ] `./gradlew ktlintCheck` passes.
- [ ] `./gradlew test --tests "*SubjectAccessRequestServiceTest*"` passes.
- [ ] `./scripts/local-scripts/regenerate-sar-snapshots.sh` completes without errors; both SAR fixtures update.
- [ ] `sar-expected-render-result.html` no longer contains `<p>No Data Held</p>` immediately below any of these three headings. Confirm with:
  ```bash
  for h in "Programme Group Memberships" "Cohort History" "LDC History"; do
    echo "== $h =="
    grep -A 3 "<h3>$h</h3>" src/test/resources/sar/sar-expected-render-result.html | head -5
  done
  ```
- [ ] `sar-expected-render-result.html` now shows Branch 2's `Attended - Complied` text and Branch 4's per-attendance layout (`<div class="attendance-record">`, `<td>Outcome</td>`, `<td>Legitimate Absence</td>`, `<td>Recorded At</td>`). Confirm with:
  ```bash
  grep -c "attendance-record\|Attended - Complied" src/test/resources/sar/sar-expected-render-result.html
  ```
  Should return at least 2.
- [ ] `sar-api-response.json` diff vs `main` is minimal — no shape changes, only ordering shifts inside `programmeGroupMemberships` and `referralLdcHistories`.
- [ ] `./gradlew test --tests "*SarContractIntegrationTest*"` passes without `SAR_GENERATE_ACTUAL` (rerun once if cohort-flake fires — override #6).
...
```bash
git add -A
git commit --no-gpg-sign -m "APG-2580: Fix SAR sections silently collapsing to No Data Held"
git push -u origin APG-2580/render-test-seed-attendance-row
```

Open a PR against `main` titled: `APG-2580 Fix SAR sections silently collapsing to "No Data Held" (community)`

Include in the PR description:

- **Production bug fix.** Since PR #644, real Community SARs have been silently omitting Programme Group Memberships, Cohort History, and LDC History sections from the human-readable HTML/PDF report even when data exists. JSON SAR endpoint was unaffected.
- **Root cause.** Mustache `{{#field.0}}` idiom evaluates falsy on `MutableSet` (no `[0]` accessor). Fields were declared `MutableSet` on the SAR DTO.
- **Fix.** Three DTO field types changed to `MutableList`; mapper calls updated; `.sortedWith(...)` safeguards added on two of the three (third already had one from Branch 1).
- **Stacks on:** PRs #860, #868, #871, #872, #874 (all merged).
- **Also picks up snapshot coverage** for Branch 2's `outcomeDescription` and Branch 4's per-attendance div, because Programme Group Memberships now renders end-to-end.
...
PRE-FLIGHT GREP RESULTS:
  - `MutableSet` on the three fields anywhere in src/main: <count or "none">
  - Test-side references to the three fields in SubjectAccessRequestServiceTest: <list or "none">
  - HmppsSubjectAccessRequestControllerIntegrationTest references: <list or "none">
  - Mustache `.0` idiom on the three fields: <count, should be 3>
...
FIXTURE DIFF SUMMARY:
  - sar-api-response.json: <one-line: only ordering changes in these two fields, or full description if anything else changed>
  - sar-expected-render-result.html: <one-line: which of the three sections now render real data + confirmation Branch 2/4 markers appear>
