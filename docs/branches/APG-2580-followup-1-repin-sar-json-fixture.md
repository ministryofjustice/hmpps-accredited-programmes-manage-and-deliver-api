...
Run `scripts/local-scripts/regenerate-sar-snapshots.sh` after the code changes. The script regenerates **three** snapshot files (verified 2026-08-24 by reading the script):
- `src/test/resources/sar/entity-schema-snapshot.json` (from `entity-schema.json.log`)
- `src/test/resources/sar/sar-api-response.json` (from `sar-api-response.json.log`)
- `src/test/resources/sar/sar-expected-render-result.html` (from `sar-generated-report.html.log`)

Expected diff for **this** ticket: `sar-api-response.json` — cohort ordering may shift by one position; other sections unchanged. `entity-schema-snapshot.json` — should be unchanged (this ticket touches no entity fields). `sar-expected-render-result.html` — see §4 below.

**Commit all three regenerated files** if any of them differ from `main`. Delete tracker override #6 in the same PR.
...
# Expect 3 hits AFTER PR #877 merges (all three sorts present).
# On raw main pre-PR#877 you would see only 1 hit (cohort only). If you get 1,
# verify with `git log --oneline main | head -5` that PR #877 has actually merged.
# Do not proceed with 1 hit — the base branch is wrong.
...
- [ ] `entity-schema-snapshot.json` unchanged (this ticket touches no entity fields; if it did change, investigate — it's a signal that something unexpected shifted).
