...
fun formatTimeOfSession(startTime: LocalTime, endTime: LocalTime, capitaliseMidday: Boolean = false): String {
  ...
  val formattedTime = "${formatTime(startTime)} to ${formatTime(endTime)}"
  return if (capitaliseMidday) formattedTime.replaceFirstChar(Char::uppercase) else formattedTime
}
