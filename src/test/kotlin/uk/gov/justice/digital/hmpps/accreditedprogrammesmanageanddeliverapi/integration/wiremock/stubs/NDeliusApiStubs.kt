...
class NDeliusApiStubs {
  ...
  fun stubSuccessfulLicenceConditionsResponse(
    crn: String,
    licenceConditions: LicenceConditions = LicenceConditions(content = listOf(LicenceConditionFactory().produce())),
  ) {
    wiremock.stubFor(
  ...
  fun stubSuccessfulRequirementsResponse(
    crn: String,
    requirements: Requirements = Requirements(content = listOf(RequirementFactory().produce())),
  ) {
    wiremock.stubFor(
  ...
  fun stubRegionWithAccreditedProgrammesMembersResponse(
    regionCode: String? = randomUppercaseString(),
    regionDto: RegionDto? = RegionDtoFactory().produce(),
  ) {
    wiremock.stubFor(
      get(urlEqualTo("/regions/$regionCode/local-admin-units/accredited-programmes/members"))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withHeader("Content-Type", "application/json")
            .withBody(objectMapper.writeValueAsString(regionDto)),
        ),
    )
  }
